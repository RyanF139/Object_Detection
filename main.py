...
# 0 = gunakan resolusi asli kamera
# >0 = resize proporsional hingga lebar <= nilai ini
...
# Format LINE dari API:
#   line         : [[x1,y1],[x2,y2]]
#   line_in_dir  : "A" | "B"
#                  Sisi asal yang dihitung sebagai IN (seperti Hikvision).
#                  Sisi A = kiri garis (cross product > 0).
#                  Sisi B = kanan garis (cross product < 0).
#                  Jika line_in_dir="A" → objek dari A ke B = IN, B ke A = OUT.
#                  Jika line_in_dir="B" → objek dari B ke A = IN, A ke B = OUT.
#   line_enabled    : true | false
#   vehicle_enabled : true | false
#                  Enable  : ROI wajib aktif, line_enabled ikut nilai API.
#                  Disable : ROI non-aktif, line paksa non-aktif.
...
CLASS_CONFIG = {
    ...
    "motorcycle": {
        ...
        "min_size": int(os.getenv("MOTORCYCLE_MIN_SIZE", 0)),
        "max_size": int(os.getenv("MOTORCYCLE_MAX_SIZE", 0)),
...
# preview_frames hanya boleh dimodifikasi di bawah preview_lock.
# camera_manager membersihkan entry yang sudah tidak aktif setiap siklus,
# sehingga display grid otomatis menyusut saat kamera dihapus dari endpoint.
preview_frames = {}
preview_lock   = Lock()
...
def point_side_of_line(px, py, x1, y1, x2, y2):
    """
    Cross product untuk menentukan sisi titik (px,py) terhadap garis (x1,y1)-(x2,y2).
    > 0 = sisi kiri (A), < 0 = sisi kanan (B), = 0 = tepat di garis.
    """
...
def check_line_cross(history, line_pts, line_in_dir):
    ...
    # Hitung sisi untuk semua history point
    sides = [
        point_side_of_line(px, py, lx1, ly1, lx2, ly2)
        for px, py in history
    ]

    # Cari crossing terakhir yang terjadi di history
    last_direction = None
    for i in range(1, len(sides)):
        prev = sides[i - 1]
        curr = sides[i]

        # Skip jika salah satu tepat di garis
        if prev == 0 or curr == 0:
            continue

        # Tidak ada crossing jika sisi sama
...
def parse_line_in_dir(raw_dir):
    """Validasi nilai line_in_dir dari API. Hanya menerima 'A' atau 'B'."""
    if raw_dir in ("A", "B"):
        return raw_dir
...
class ObjectTracker:
    """Tracks objects across frames by proximity of bounding-box centres."""
...
def build_session(model_path: str) -> ort.InferenceSession:
    ...
    print(f"[ONNX] Provider aktif: {sess.get_providers()}")
    return sess
...
class CameraWorker:
    ...
    def update_config(self, roi=None, line=None, line_in_dir=None,
                      line_enabled=None, vehicle_enabled=None):
        """
        Hot-reload konfigurasi kamera dari API.

        Rules:
        - vehicle_enabled=True  → ROI aktif, line_enabled ikut nilai API
        - vehicle_enabled=False → ROI non-aktif, line paksa False
        """
        changed = False
    ...
    def _process_object(self, obj, frame_original, view,
        ...
        if cls_name in VEHICLE_CLASSES:
            # vehicle_enabled = False → abaikan semua kendaraan
            if not self.vehicle_enabled:
                return False

            # vehicle_enabled = True → butuh line aktif untuk crossing
            ...
            # Syarat Utama Urutan: Titik tengah kendaraan HARUS berada di dalam ROI pada frame SEBELUM menyeberangi garis (Urutan ROI -> LINE)
            if len(track_data["history"]) >= 2:
                ...
                if not prev_in_roi:
                    # Pergerakan LINE -> ROI -> TOLAK TOTAL!
                    return False
        ...
        # ── Crop & encode dari frame_original ────────────────────────────────
        cx1, cy1, cx2, cy2 = expand_crop_bbox(ox1, oy1, ox2, oy2, orig_w, orig_h)
        crop_original      = frame_original[cy1:cy2, cx1:cx2]
    ...
    def run(self):
        while self.running:
            ...
            raw_dets_infer = postprocess(outputs, scale, inf_w, inf_h)
            ...
            if self.vehicle_enabled:
                ...
                # ROI non-aktif → abu-abu sebagai referensi visual
                cv2.polylines(view, [roi_scaled], True, (80, 80, 80), WIDTH_LINE)

            # ── Overlay: LINE & status vehicle ───────────────────────────────
            if not self.vehicle_enabled:
                cv2.putText(
                ...
                # Sisi A = kiri garis → selalu "IN"
                # Sisi B = kanan garis → selalu "OUT"
                ...
                (wa, ha), _ = cv2.getTextSize("IN",  FONT, FONT_SCALE, THICKNESS)
                (wb, hb), _ = cv2.getTextSize("OUT", FONT, FONT_SCALE, THICKNESS)
            ...
            # ── Update preview (hanya jika worker masih running) ─────────────
            if self.running:
                with preview_lock:
...
def camera_manager():
    ...
    while True:
        ...
        with camera_lock:
            ...
            # ── Sinkronkan preview_frames dengan active_cameras ───────────────
            # Hapus entry kamera yang sudah tidak aktif agar grid display
            # otomatis menyusut tanpa perlu restart.
            with preview_lock:
...
if ENABLE_VIEW:
    ...
    while True:
        with preview_lock:
            # Snapshot konsisten: ambil frames hanya dari kamera yang masih terdaftar
            snapshot = list(preview_frames.values())

        if not snapshot:
            # Tidak ada kamera aktif — tampilkan layar hitam dengan pesan
        ...
        # Pad sisa slot grid dengan frame hitam
