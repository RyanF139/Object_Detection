...
def check_line_cross(history, line_pts, line_in_dir):
    """
    Cek crossing dari seluruh history, kembalikan crossing TERBARU.
    Return None jika tidak ada crossing baru sejak last_cross_dir.
    """
    if len(history) < 2:
        return None
        ...
        for (x1, y1, x2, y2, conf, cls_name) in detections:
            ...
            if best_id is not None:
                ...
                self.tracks[new_id] = {
                    ...
                    "sent_once":      True,
                    "matched":        True,
                    "last_cross_dir": None,  # ← track crossing terakhir yg sudah dikirim
                }
                enriched.append({
                    "bbox":        (x1, y1, x2, y2),
                    "conf":        conf,
...
def postprocess(outputs, scale: float, orig_w: int, orig_h: int):
    ...
    # Kumpulkan deteksi per class agar NMS tidak saling suppress antar class
    ...
    # NMS per class — person dan vehicle tidak saling suppress
    results = []
