...
# ================= LOGGING SETUP =================
# Save the original print function before overriding so we can print clean startup banners without logging prefixes
_orig_print = print

# Set up a structured logger to make terminal logs readable and formatted
logging.basicConfig(
    level=logging.INFO,
    format="[%(asctime)s] %(levelname)s [%(name)s] %(message)s",
    datefmt="%Y-%m-%d %H:%M:%S"
)
logger = logging.getLogger("DetectionApp")

# Override print dynamically to redirect all logs to the logger
def print(*args, **kwargs):
    msg = " ".join(str(arg) for arg in args).strip()
    if not msg:
        return
    msg_upper = msg.upper()
    if any(x in msg_upper for x in ["ERROR", "FAIL", "GAGAL", "EXCEPTION"]):
        logger.error(msg)
    elif any(x in msg_upper for x in ["WARNING", "WARN", "RECONNECT"]):
        logger.warning(msg)
    elif any(x in msg_upper for x in ["DEBUG", "VERBOSE"]):
        logger.debug(msg)
    else:
        logger.info(msg)
...
def print_banner():
    try:
        ...
        f = Figlet(font='slant')
        banner = f.renderText('VIDEO CONTENT ANALYSIS')
        _orig_print(colored(banner, 'green'), flush=True)
    except Exception:
        # Fallback jika pyfiglet bermasalah (misal font tidak ketemu di docker/alpine)
        try:
            from termcolor import colored
    ...
    try:
        from termcolor import colored
        _orig_print(colored("VIDEO CONTENT ANALYSIS v1.0.4", 'blue'), flush=True)
    except Exception:
        _orig_print("VIDEO CONTENT ANALYSIS v1.0.4")

def print_system_status():
    gpu_label = "GPU" if DEVICE == "cuda" else "CPU"
    detail_label = f"device={CUDA_DEVICE_ID}" if DEVICE == "cuda" else "device=cpu"
    
    _orig_print("\n==================================================")
    _orig_print(f"{'AI SERVICE':<25} | {'DEVICE':<10} | {'DETAIL'}")
    _orig_print("--------------------------------------------------")
    _orig_print(f"{'Vehicle Detection':<25} | {gpu_label:<10} | {detail_label}")
    _orig_print(f"{'Face Detection':<25} | {gpu_label:<10} | {detail_label}")
    _orig_print(f"{'Weapon Detection':<25} | {gpu_label:<10} | {detail_label}")
    _orig_print("==================================================\n")
...
# if "yolov8" in FACE_MODEL_PATH or not os.path.exists(FACE_MODEL_PATH):
#     if os.path.exists("models_lib/face_detection_yunet_2023mar.onnx"):
#         FACE_MODEL_PATH = "models_lib/face_detection_yunet_2023mar.onnx"
WEAPON_MODEL_PATH = os.getenv("WEAPON_MODEL_PATH", "models/mhp_weaponv1.onnx")
if not os.path.exists(WEAPON_MODEL_PATH):
    if os.path.exists("models_lib/mhp_weaponv1.onnx"):
        WEAPON_MODEL_PATH = "models_lib/mhp_weaponv1.onnx"
...
_ACTIVE_GPU_PROVIDER = None

def _check_onnxruntime_cuda() -> bool:
    global _ACTIVE_GPU_PROVIDER
    available = ort.get_available_providers()
    
    # Prioritaskan CUDAExecutionProvider karena paling umum dan stabil
    gpu_providers = ["CUDAExecutionProvider", "DmlExecutionProvider", "ROCMExecutionProvider", "TensorrtExecutionProvider"]
    
    for provider in gpu_providers:
        if provider in available:
            try:
                # Tes loading nyata untuk memverifikasi ketersediaan DLL (seperti nvinfer_10.dll atau cudnn64_8.dll)
                if provider == "CUDAExecutionProvider":
                    prov_opt = [("CUDAExecutionProvider", {"device_id": CUDA_DEVICE_ID})]
                elif provider == "TensorrtExecutionProvider":
                    prov_opt = [("TensorrtExecutionProvider", {"device_id": CUDA_DEVICE_ID})]
                else:
                    prov_opt = [provider]
                
                # Cari model yang ada untuk tes loading nyata guna memverifikasi ketersediaan DLL/CUDA
                model_to_test = None
                for p in [MODEL_PATH, FACE_MODEL_PATH, WEAPON_MODEL_PATH]:
                ...
                if model_to_test:
                    ...
                    with suppress_stdout_stderr():
                        test_sess = ort.InferenceSession(model_to_test, providers=prov_opt, sess_options=opts)
                    active_providers = test_sess.get_providers()
                    del test_sess
                    
                    # ONNX Runtime sering kali melakukan fallback internal ke CPU secara diam-diam tanpa exception jika DLL gagal dimuat.
                    # Kita harus secara eksplisit memverifikasi bahwa provider yang diinginkan ada di daftar provider yang aktif.
                    if provider in active_providers:
                        _ACTIVE_GPU_PROVIDER = provider
                        print(f"[CUDA CHECK] onnxruntime: {provider} BERHASIL terverifikasi aktif pada hardware!")
                        return True
                    else:
                        print(f"[CUDA CHECK] onnxruntime: {provider} terdaftar di ORT namun gagal dimuat secara aktif (fallback ke CPU terjadi).")
                else:
                    # Jika tidak ada file model untuk tes, asumsikan provider aktif karena terdaftar di available
                    _ACTIVE_GPU_PROVIDER = provider
                    print(f"[CUDA CHECK] onnxruntime: {provider} terdaftar di ORT. Model file belum tersedia untuk pengetesan beban, diasumsikan aktif.")
                    return True
            except Exception as e:
                print(f"[CUDA CHECK] onnxruntime: Gagal memuat '{provider}' (Pustaka pendukung tidak lengkap/tidak kompatibel). Error: {e}")
        
    print("[CUDA CHECK] onnxruntime: Tidak ada provider GPU (CUDA/DirectML/TensorRT) yang berfungsi penuh di environment Python ini.")
    print("             -> TIPS: Untuk performa GPU maksimal di virtual environment, silakan jalankan:")
...
def _check_opencv_cuda() -> bool:
    ...
    try:
        # Cek apakah backend CUDA didukung di cv2.dnn tanpa memuat file model (lebih aman & cepat)
        backends = cv2.dnn.getAvailableBackends()
        if cv2.dnn.DNN_BACKEND_CUDA in backends:
            print("[CUDA CHECK] OpenCV DNN CUDA backend: OK (terverifikasi dari list available backends)")
            return True
        else:
            print("[CUDA CHECK] OpenCV DNN CUDA backend gagal: backend CUDA tidak didukung oleh build OpenCV ini")
            return False
    except Exception as e:
...
def _resolve_device() -> str:
    device_env_clean = _DEVICE_ENV.strip().lower()
    if device_env_clean == "cpu":
        print("[DEVICE] Mode: CPU (dipaksa lewat .env DEVICE=cpu)")
        return "cpu"

    print("[DEVICE] Memeriksa ketersediaan GPU/CUDA secara otomatis...")
    ort_ok = _check_onnxruntime_cuda()

    if ort_ok:
        print(f"[DEVICE] Mode: FULL GPU - onnxruntime {_ACTIVE_GPU_PROVIDER} (device_id={CUDA_DEVICE_ID})")
        return "cuda"
    else:
        print("[DEVICE] Mode: CPU (semua akselerasi GPU gagal, fallback otomatis ke CPU)")
        return "cpu"
...
_ORT_USE_CUDA = DEVICE == "cuda"
_OCV_USE_CUDA = _check_opencv_cuda()
...
WEAPON_CONF         = float(os.getenv("WEAPON_CONF",         0.45))
WEAPON_COOLDOWN     = float(os.getenv("WEAPON_COOLDOWN",     10.0))
WEAPON_COLOR        = tuple(map(int, os.getenv("WEAPON_COLOR", "0,0,255").split(",")))

SAVE_FOLDER        = os.getenv("SAVE_FOLDER",   "image_detection")
FACE_FOLDER_BASE   = os.getenv("FACE_FOLDER",   "image_face")
WEAPON_FOLDER_BASE = os.getenv("WEAPON_FOLDER", "image_weapon")
...
SAVE_IMAGE_WEAPON  = os.getenv("SAVE_IMAGE_WEAPON",  "true").lower() == "true"
...
WEAPON_IMG_SIZE  = int(os.getenv("WEAPON_IMG_SIZE",  1280))
...
DEFAULT_WEAPON_ENABLED  = os.getenv("WEAPON_ENABLED",  "true").lower() == "true"
DEFAULT_SAVE_IMAGE_VEHICLE = SAVE_IMAGE and SAVE_IMAGE_VEHICLE
DEFAULT_SAVE_IMAGE_FACE    = SAVE_IMAGE and SAVE_IMAGE_FACE
DEFAULT_SAVE_IMAGE_WEAPON  = SAVE_IMAGE and SAVE_IMAGE_WEAPON
...
WEAPON_CROP_FOLDER  = os.path.join(WEAPON_FOLDER_BASE, "weapon")
WEAPON_FRAME_FOLDER = os.path.join(WEAPON_FOLDER_BASE, "frame")

for _d in [DETECT_FOLDER, FRAME_FOLDER_VEH, FACE_CROP_FOLDER, FACE_FRAME_FOLDER, WEAPON_CROP_FOLDER, WEAPON_FRAME_FOLDER]:
...
weapon_webhook_queue = Queue(maxsize=1000)
...
def build_shared_session():
    ...
    if _ORT_USE_CUDA and _ACTIVE_GPU_PROVIDER:
        if _ACTIVE_GPU_PROVIDER == "CUDAExecutionProvider":
            providers = [("CUDAExecutionProvider", {"device_id": CUDA_DEVICE_ID}), "CPUExecutionProvider"]
        elif _ACTIVE_GPU_PROVIDER == "TensorrtExecutionProvider":
            providers = [
                ("TensorrtExecutionProvider", {"device_id": CUDA_DEVICE_ID}),
                ("CUDAExecutionProvider", {"device_id": CUDA_DEVICE_ID}),
                "CPUExecutionProvider"
            ]
        else:
            providers = [_ACTIVE_GPU_PROVIDER, "CPUExecutionProvider"]
    else:
...
# ================= SHARED WEAPON SESSION =================

_shared_weapon_session    = None
_shared_weapon_input_name = None
_weapon_session_lock      = Lock()


def build_shared_weapon_session():
    opts = ort.SessionOptions()
    opts.intra_op_num_threads     = ONNX_INTRA_THREADS
    opts.inter_op_num_threads     = ONNX_INTER_THREADS
    opts.execution_mode           = ort.ExecutionMode.ORT_SEQUENTIAL
    ...
    sess       = ort.InferenceSession(WEAPON_MODEL_PATH, providers=providers, sess_options=opts)
    input_name = sess.get_inputs()[0].name

    dummy = np.zeros((1, 3, WEAPON_IMG_SIZE, WEAPON_IMG_SIZE), dtype=np.float32)
    sess.run(None, {input_name: dummy})

    print(f"[ONNX WEAPON] provider={sess.get_providers()[0]} | device_id={CUDA_DEVICE_ID}")
    return sess, input_name


def get_shared_weapon_session():
    global _shared_weapon_session, _shared_weapon_input_name
    with _weapon_session_lock:
        if _shared_weapon_session is None:
            _shared_weapon_session, _shared_weapon_input_name = build_shared_weapon_session()
    return _shared_weapon_session, _shared_weapon_input_name


print(f"[STARTUP] Inisialisasi Weapon session | device={DEVICE.upper()}...")
get_shared_weapon_session()
print("[STARTUP] Weapon session siap")

# ================= FACE INFERENCE WORKERS =================

_vehicle_run_lock = Lock()
_weapon_run_lock = Lock()
...
# ================= WEAPON INFERENCE WORKERS =================

_weapon_infer_queue: Queue = Queue(maxsize=50)


def weapon_inference_worker(worker_id: int):
    sess, input_name = get_shared_weapon_session()
    print(f"[WEAPON WORKER {worker_id}] Started | provider={sess.get_providers()[0]}")
    while True:
        try:
            item = _weapon_infer_queue.get(timeout=1.0)
        except Empty:
            continue
        if item is None:
            break
        cid, frame_infer, result_q = item
        try:
            inp, scale = preprocess_weapon(frame_infer)
            with _weapon_run_lock:
                outputs    = sess.run(None, {input_name: inp})
            h, w       = frame_infer.shape[:2]
            results    = postprocess_weapon(outputs, scale, w, h)
            result_q.put(("ok", results))
        except Exception as e:
            result_q.put(("err", []))
            print(f"[WEAPON WORKER {worker_id}] Error cam={cid}: {e}")
        finally:
            _weapon_infer_queue.task_done()


for _i in range(_INFER_WORKERS):
    Thread(target=weapon_inference_worker, args=(_i,), daemon=True).start()

print(f"[STARTUP] {_INFER_WORKERS} weapon inference worker(s) started")
...
def preprocess_weapon(img: np.ndarray):
    h, w    = img.shape[:2]
    scale   = min(WEAPON_IMG_SIZE / w, WEAPON_IMG_SIZE / h)
    nw, nh  = int(w * scale), int(h * scale)
    resized = cv2.resize(img, (nw, nh), interpolation=cv2.INTER_AREA)
    canvas  = np.full((WEAPON_IMG_SIZE, WEAPON_IMG_SIZE, 3), 114, dtype=np.uint8)
    canvas[:nh, :nw] = resized
    inp = canvas.astype(np.float32) / 255.0
    inp = np.transpose(inp, (2, 0, 1))
    inp = np.expand_dims(inp, axis=0)
...
# preprocess_face removed


def postprocess_weapon(outputs, scale: float, orig_w: int, orig_h: int):
    out = outputs[0]
    ...
    # Class mapping for mhp_weaponv1: {0: 'person', 1: 'weapon'}
    # We only care about class index 1 ('weapon')
    for row in out:
        conf = float(row[5])
        if conf < WEAPON_CONF:
            continue
    ...
    indices = cv2.dnn.NMSBoxes(boxes, scores, WEAPON_CONF, IOU_THRESHOLD)
    results = []
    if len(indices) > 0:
        for i in indices.flatten():
            x, y, w, h = boxes[i]
            results.append((x, y, x + w, y + h, scores[i], "weapon"))
    return results
...
def weapon_webhook_worker():
    while True:
        item = weapon_webhook_queue.get()
        if item is None:
            break
        try:
            weapon_bytes, frame_bytes, weapon_name, frame_name, \
                name_camera, ts_iso, bbox, score, cid, client_id = item

            payload = {
                "timestamp":  ts_iso,
                "type":       "weapon_detection_service",
                "class_name": "weapon",
                "bbox":       f"{bbox[0]},{bbox[1]},{bbox[2]},{bbox[3]}",
                "confidence": round(score, 4),
                "channel_id": cid,
            ...
            print(f"\n[WEBHOOK WEAPON] {json.dumps({'url': WEBHOOK_URL, 'frame': frame_name, 'data': payload}, indent=2)}")
            resp = requests.post(
                WEBHOOK_URL,
                files=[
                    ("files", (frame_name,  frame_bytes,  "image/jpeg")),
                    ("files", (weapon_name, weapon_bytes, "image/jpeg")),
                ],
                data=payload,
                timeout=10,
            )
            resp.raise_for_status()
            print(f"[WEBHOOK WEAPON OK] {weapon_name} | conf={score:.3f} | http={resp.status_code}")
        except Exception as e:
            print(f"[WEBHOOK WEAPON FAIL] {e}")
        finally:
            weapon_webhook_queue.task_done()


if WEBHOOK_URL:
    Thread(target=vehicle_webhook_worker, daemon=True).start()
    Thread(target=face_webhook_worker,    daemon=True).start()
    Thread(target=weapon_webhook_worker,  daemon=True).start()
...
class CameraWorker:
    ...
    def __init__(self, cid, client_id, url, name,
                 ...
                 face_enabled=None, weapon_enabled=None,
                 save_image_vehicle=None, save_image_face=None,
                 save_image_weapon=None):
        ...
        self.weapon_enabled = bool(weapon_enabled) if weapon_enabled is not None else DEFAULT_WEAPON_ENABLED
        self.weapon_memory  = {}
        ...
        self.save_image_weapon = (
            bool(save_image_weapon) if save_image_weapon is not None else DEFAULT_SAVE_IMAGE_WEAPON
        )
        ...
        if self.connected:
            Thread(target=self._frame_reader_loop, daemon=True).start()

        print(f"[CAMERA START] {cid} -> {name} | vehicle={self.vehicle_enabled} | face={self.face_enabled} | weapon={self.weapon_enabled}")
    ...
    def update_config(self, roi=None, line=None, line_in_dir=None,
                      line_enabled=None, vehicle_enabled=None, face_enabled=None, weapon_enabled=None,
                      save_image_vehicle=None, save_image_face=None, save_image_weapon=None):
        changed = False
    ...
    def adaptive_fps(self):
        idle = (
            ...
            time.time() - self.last_face_time   > IDLE_TIMEOUT and
            time.time() - self.last_weapon_time > IDLE_TIMEOUT
        )
    ...
    def cleanup_weapon_memory(self):
        now   = time.time()
        stale = [k for k, v in self.weapon_memory.items() if now - v[0] > 60]
        for k in stale:
    ...
    def _submit_for_weapon_inference(self, frame_infer) -> bool:
        while not self._weapon_result_q.empty():
            try:
                self._weapon_result_q.get_nowait()
            except Empty:
                break
        try:
            _weapon_infer_queue.put_nowait((self.cid, frame_infer, self._weapon_result_q))
            return True
        except Exception:
            return False

    def _get_weapon_result(self, timeout=0.5):
        try:
            status, results = self._weapon_result_q.get(timeout=timeout)
            if status == "ok":
                return results
        except Empty:
            pass
    ...
    def _run_weapon_detection(self, frame_original, view):
        if not self.weapon_enabled:
            return

        frame_infer = self.resize_frame(frame_original)
        if not self._submit_for_weapon_inference(frame_infer):
            return

        results = self._get_weapon_result(timeout=0.5)
        if not results:
            return
        ...
        for (x1, y1, x2, y2, score, cls_name) in results:
            ...
            # Bucket memory (using 220 grid size as in face detection)
            bucket = (cx // 220, cy // 220)
            now    = time.time()

            if bucket in self.weapon_memory:
                last_time, last_pos = self.weapon_memory[bucket]
                if now - last_time < WEAPON_COOLDOWN:
                    continue
                # Move threshold (like face detection move threshold)
                if math.hypot(cx - last_pos[0], cy - last_pos[1]) < 80:
                    continue

            self.weapon_memory[bucket] = (now, (cx, cy))
            self.last_weapon_time = time.time()

            # Crop weapon image and clean frame
            wx1, wy1, wx2, wy2 = expand_crop_bbox(ox1, oy1, ox2, oy2, orig_w, orig_h, CROP_PADDING)
            weapon_img = frame_original[wy1:wy2, wx1:wx2]

            if weapon_img.size == 0:
                continue

            # Overlay bounding box on view
            vscale = view.shape[1] / orig_w
            vx1, vy1 = int(ox1 * vscale), int(oy1 * vscale)
            vx2, vy2 = int(ox2 * vscale), int(oy2 * vscale)
            cv2.rectangle(view, (vx1, vy1), (vx2, vy2), WEAPON_COLOR, WIDTH_LINE)
            cv2.putText(view, f"weapon {score:.2f}",
                        (vx1, max(0, vy1 - 10)),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.50, WEAPON_COLOR, WIDTH_LINE)

            ts_iso      = iso_timestamp()
            weapon_name = iso_name("weapon", "weapon", ts_iso)
            frame_name  = iso_name("frame",  "weapon", ts_iso)

            wb1 = cv2.imencode(".jpg", weapon_img)[1].tobytes()
            wb2 = cv2.imencode(".jpg", frame_original)[1].tobytes()

            if self.save_image_weapon:
                try:
                    with open(os.path.join(WEAPON_CROP_FOLDER,  weapon_name), "wb") as fp: fp.write(wb1)
                    with open(os.path.join(WEAPON_FRAME_FOLDER, frame_name),  "wb") as fp: fp.write(wb2)
                    enforce_limit(WEAPON_CROP_FOLDER)
                    enforce_limit(WEAPON_FRAME_FOLDER)
                except Exception as e:
                    print(f"[SAVE ERROR WEAPON] {e}")
            else:
                print(f"[SKIP SAVE WEAPON] {frame_name}")

            if WEBHOOK_URL:
                try:
                    weapon_webhook_queue.put_nowait((
                        wb1, wb2, weapon_name, frame_name,
                        self.name_camera, ts_iso,
                        (ox1, oy1, ox2 - ox1, oy2 - oy1), score,
                        self.cid, self.client_id,
                    ))
                except Exception:
                    print("[QUEUE FULL] weapon webhook queue penuh")

            print(f"[{self.cid}] WEAPON SAVED {weapon_name} | conf={score:.3f}")
    ...
    def run(self):
        while self.running:
            ...
            # ── Weapon detection ───────────────────────────────────────
            self._run_weapon_detection(frame_original, view)
            self.cleanup_weapon_memory()
            ...
            if not self.weapon_enabled:
                cv2.putText(view, "WEAPON: OFF", (8, y_off + 20),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.45, (100, 100, 100), WIDTH_LINE)
            ...
            cv2.putText(view, mode_label, (8, view.shape[0] - 60),
            ...
            siw_color = (0, 200, 0) if self.save_image_weapon  else (80, 80, 80)
            cv2.putText(view, "SAVE VEH: ON"  if self.save_image_vehicle else "SAVE VEH: OFF",
                        (8, view.shape[0] - 45), cv2.FONT_HERSHEY_SIMPLEX, 0.40, siv_color, WIDTH_LINE)
            cv2.putText(view, "SAVE FACE: ON" if self.save_image_face    else "SAVE FACE: OFF",
                        (8, view.shape[0] - 30), cv2.FONT_HERSHEY_SIMPLEX, 0.40, sif_color, WIDTH_LINE)
            cv2.putText(view, "SAVE WEAPON: ON" if self.save_image_weapon else "SAVE WEAPON: OFF",
                        (8, view.shape[0] - 15), cv2.FONT_HERSHEY_SIMPLEX, 0.40, siw_color, WIDTH_LINE)
...
def print_camera_status():
    with camera_lock:
        ...
        save_weapon_on = sum(1 for w in active_cameras.values() if w.save_image_weapon)
    ...
    print(f"Weapon Wbhk Queue : {weapon_webhook_queue.qsize()}")
    print(f"Vehicle Infer Q   : {_infer_input_queue.qsize()}")
    print(f"Face Infer Q      : {_face_infer_queue.qsize()}")
    print(f"Weapon Infer Q    : {_weapon_infer_queue.qsize()}")
    print()
    ...
    print(f"Save Img Weapon   : {save_weapon_on}/{total} ON")
    print("==================================================\n")
...
if ENABLE_VIEW:
    try:
        ...
        print("\n[GUI WARNING] Pustaka OpenCV tidak mendukung Window (Headless). Tampilan visual otomatis dinonaktifkan.")
        print("              Sistem otomatis berjalan dalam mode headless (background).")
        ENABLE_VIEW = False
